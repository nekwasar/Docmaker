...
export default function SummarizeScreen() {
  ...
  const handleSummarize = () => {
    ...
    setTimeout(() => { setSummary('This document provides a comprehensive overview of the key topics. The main points include project objectives, timeline, budget allocation, and expected outcomes.'); setIsProcessing(false); }, 2000);
  };
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(300)}>
            <View style={styles.card}>
              <View style={styles.fileRow}>
                ...
                <View style={{ flex: 1 }}><Text style={styles.fileName}>report.pdf</Text><Text style={styles.fileMeta}>10 pages</Text></View>
            ...
            <AnimatedPressable onPress={handleSummarize} haptic="medium" style={[styles.actionBtn, isProcessing && styles.actionBtnDisabled]} disabled={isProcessing}>
              {isProcessing ? <Text style={styles.actionBtnText}>Summarizing...</Text> : <><Ionicons name="reader" size={20} color={Colors.white} /><Text style={styles.actionBtnText}>Summarize Document</Text></>}
            </AnimatedPressable>
            {summary ? (
              <View style={styles.card}>
                <View style={styles.answerHeader}><Ionicons name="document-text" size={16} color={Brand.navy} /><Text style={styles.answerLabel}>Summary</Text></View>
                <Text style={styles.answerText}>{summary}</Text>
              </View>
            ) : null}
          </Animated.View>
        )}
