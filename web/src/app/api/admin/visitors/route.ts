...
export async function GET(req: NextRequest) {
  ...
  const [total, items, topPaths] = await Promise.all([
    ...
    prisma.pageView
      .groupBy({ by: ["path"], _count: { _all: true } })
      .then((rows) => rows.sort((a, b) => (b._count?._all ?? 0) - (a._count?._all ?? 0)).slice(0, 10)),
  ]);
  ...
  return NextResponse.json({
    ...
    limit,
    items,
    topPaths: topPaths.map((r) => ({ path: r.path, count: r._count?._all ?? 0 })),
  });
}
