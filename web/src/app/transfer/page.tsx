...
export default function TransferPage() {
  ...
  // Modal auto-redirect flow
  useEffect(() => {
    if (!showModal) return;

    // Phase 1: Wait 2 seconds, then change to "redirecting"
    const t1 = setTimeout(() => {
      setModalPhase("redirecting");
    }, 2000);

    // Phase 2: After 1 more second, redirect
    const t2 = setTimeout(() => {
  ...
  return (
    <ToolPageLayout title="File Transfer" color="yellow">
      <div className="space-y-8">
        ...
        {!session ? (
          // Not logged in — show content behind modal
        ...
        >
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 text-center">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Create an account to store and transfer files securely between your devices.
            </p>
            ...
            <div className="mt-4 h-1 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#FFD140] transition-all duration-1000 ease-linear"
                style={{ width: modalPhase === "redirecting" ? "100%" : "33%" }}
              />
            </div>
          </div>
        </div>
