...
import { Menu, X, FileText, ChevronDown, Sparkles, FileUp, Merge, Scissors, Pen, Lock, Layers, Globe, Scan, Zap, Shield, BookOpen, HelpCircle, MessageSquare } from "lucide-react";
        ...
        <nav className="hidden lg:flex items-center gap-1">
          ...
          >
            <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors rounded-lg hover:bg-slate-50">
              Tools
              <ChevronDown className={`h-4 w-4 transition-transform ${activeMega === "tools" ? "rotate-180" : ""}`} />
            </button>

            {activeMega === "tools" && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[700px] rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
          ...
          >
            <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors rounded-lg hover:bg-slate-50">
              Resources
              <ChevronDown className={`h-4 w-4 transition-transform ${activeMega === "resources" ? "rotate-180" : ""}`} />
            </button>

            {activeMega === "resources" && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[500px] rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 space-y-1">
                {TOOLS_MEGA.categories.map((cat) => (
                  <div key={cat.title}>
                    <p className="text-sm font-medium text-slate-900 mt-2">{cat.title}</p>
                    {cat.items.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="block py-1 pl-3 text-sm text-slate-500 hover:text-slate-900"
            ...
            <Link href="/pricing" className="block text-sm font-medium text-slate-900" onClick={() => setMobileOpen(false)}>
              Pricing
            </Link>
